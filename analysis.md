...
# bla <- read_excel("./data/WI.xls") #%>% 
#   select( c("LOGRECNO","GEOID")) %>% 
#   #convert GEOID to 12-digit FIPS
#   mutate(GEOID = substr(GEOID,8,20))
```

##### Clean and subset ACS data


```r
# Subset data
acs_data <- acs_data %>% 
  select("LOGRECNO","B17010_001","B17010_002") 

# Unite data and geo info
...
```r
acs <- acs %>% 
  mutate(poverty = B17010_002/B17010_001) %>% 
...
#for current address
data <- left_join(mars, acs, by=c("current_address" = "GEOID")) %>% 
  rename(poverty_current = poverty)

# for past address
data <- left_join(data, acs, by=c("previous_address" = "GEOID")) %>% 
  rename(poverty_previous = poverty)
